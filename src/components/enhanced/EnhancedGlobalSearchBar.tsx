...
interface EnhancedGlobalSearchBarProps {
  placeholder?: string;
  className?: string;
}

const EnhancedGlobalSearchBar = ({ 
  placeholder = "Search movies, series, anime...", 
  className = "" 
}: EnhancedGlobalSearchBarProps) => {
  ...
  useEffect(() => {
    const searchContent = async () => {
      ...
      try {
        ...
        // Enhanced search with fuzzy matching and SEO tags
        const searchQueries = [];
        ...
        searchQueries.push(
          supabase
            .from('movies')
            .select('movie_id, title, poster_url, year, content_type, genre, country, imdb_rating, downloads, seo_tags')
            .eq('is_visible', true)
            .ilike('title', `%${query}%`)
        ...
        // 2. Partial word matches in title
        ...
        // 3. SEO tags search - enhanced matching
        ...
        searchQueries.push(
          supabase
            .from('movies')
            .select('movie_id, title, poster_url, year, content_type, genre, country, imdb_rating, downloads, seo_tags, director, production_house')
            .eq('is_visible', true)
            .or(`director.ilike.%${query}%,production_house.ilike.%${query}%`)
            .order('downloads', { ascending: false })
            .limit(3)
        );
        ...
        // Advanced scoring for relevance - enhanced for "the big bang" matching "The Big Bang Theory"
        const scoredResults = uniqueResults.map(movie => {
          ...
          // Enhanced individual word matches - key improvement for partial matches
          searchTerms.forEach(term => {
            if (title.includes(term)) score += 25; // Increased score for word matches
            
            // Enhanced partial word matching for better results
            const titleWords = title.split(' ');
            titleWords.forEach(titleWord => {
              if (titleWord.includes(term)) score += 15;
              if (term.includes(titleWord) && titleWord.length > 2) score += 10;
              // Fuzzy matching for similar words
              if (Math.abs(titleWord.length - term.length) <= 2 && 
                  titleWord.includes(term.substring(0, Math.min(3, term.length)))) {
                score += 8;
              }
            });
          });

          // SEO tags matching - enhanced
          if (movie.seo_tags) {
            searchTerms.forEach(term => {
              movie.seo_tags!.forEach(tag => {
                const tagLower = tag.toLowerCase();
                if (tagLower.includes(term)) score += 20;
                if (tagLower === term) score += 30;
                // Partial SEO tag matching
                if (tagLower.includes(term.substring(0, Math.min(3, term.length))) && term.length > 2) {
                  score += 12;
                }
              });
            });
    ...
    const debounceTimer = setTimeout(searchContent, 200); // Reduced debounce for faster response
  ...
  return (
    <div ref={searchRef} className={`relative w-full max-w-md ${className}`}>
      ...
      {isOpen && (
        <Card className="absolute top-full left-0 right-0 mt-2 bg-gray-800 border-gray-600 z-[9999] max-h-96 overflow-y-auto shadow-xl">
